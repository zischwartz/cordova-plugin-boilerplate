...
@objc(SwiftVideoPlugin) class SwiftVideoPlugin : CDVPlugin {
  ...
  func begin(command: CDVInvokedUrlCommand) {
  		self.commandDelegate?.runInBackground({() -> Void in
            // TODO allow for for other presets/resolutons
            ...
            // Sizes evidently need to be from 
            // https://en.wikipedia.org/wiki/Common_Intermediate_Format
	        self.output = GPUImageRawDataOutput(imageSize: self.options.size!, resultsInBGRAFormat: true)
	        ...
	        self.output!.newFrameAvailableBlock = { () in
                ...
                // XXX if this is too low, javascript cb won't ever happen
	            // 300 was good at 352, 288
	            // 50 for 176, 144
	            // 0 worked at 128, 96
	            if since_last > 50 { 
